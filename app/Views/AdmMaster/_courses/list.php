...
<?= $this->section('content') ?>
    ...
    <div class="page-heading mb-4 d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2 align-items-center">
            ...
            <a href="/AdmMaster/_courses/write" class="btn btn-primary">
                    ...
                    <table class="table table-bordered align-middle">
                        ...
                        <?php foreach ($items as $key => $item): ?>
                            <tr>
                                ...
                                <td>
                                    <?= getCodeCourse($item['idx']) ?>
                                </td>
                                <td>
                                    <a href="/AdmMaster/_courses/write?idx=<?= $item['idx'] ?>"><?= $item['course_name'] ?></a>
                                </td>
                                <td><?= $item['mentor'] ?>  </td>
                                <td><?= date('Y.m.d', strtotime($item['start_date'])) ?>
                                    ~ <?= date('Y.m.d', strtotime($item['end_date'])) ?></td>
                                <td><?= $item['price'] > 0 ? $item['price'] : '무료' ?></td>
                                <td>
                                ...
                                <td>
                                    <input type="text" class="form-control" id="onum_<?= $item['idx'] ?>" name="onum[]"
                                           value="<?= $item['onum'] ?>">
                                    <input type="hidden" name="idx[]" id="idx_<?= $item['idx'] ?>"
                                           value="<?= $item['idx'] ?>">
                                </td>
                                <td>
                                    <div class="d-flex justify-content-center align-items-center gap-2">
                                        <a href="/AdmMaster/_courses/write?idx=<?= $item['idx'] ?>"
    ...
    <script>
        ...
        async function processDelete(formData) {
            await fetch('<?= site_url('AdmMaster/_courses/delete') ?>', {
                method: "POST",
                body: formData,
            })
        ...
        async function change_it() {
            let url = '<?= site_url('AdmMaster/_courses/change') ?>';
