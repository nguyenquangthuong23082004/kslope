...
$total_sql = " select *		
						from tbl_join where 1=1 $strSql $dateSql $status_Sql $type_Sql $deli_Sql";
...
?>
		<div class="page-heading mb-4">
			...
			<div class="d-flex justify-content-between align-items-center">
				...
				<h4 class="text-center">제품가입신청</h4>
			</div>
		</div>
    <div id="container" class="gnb_inquiry">
		<div id="print_this"><!-- 인쇄영역 시작 //--><!-- // headerContainer -->
			...
			<div id="contents">
				...
				<div class="listWrap">
					...
					<div class="listBottom">
						...
						<tbody>
                            ...
                            while ($row = mysqli_fetch_array($result)) {
								...
								<td><?= $num-- ?></td>
								<td class="tac"><?= ($row['join_type'] == 1 ? "가입신청" : ($row['join_type'] == 2 ? "다이렉트" : "")) ?></td>
								<td class="tac"><?= ($row['business'] == 'p' ? "개인" : ($row['business'] == 'c' ? "법인" : ($row['business'] == 'n' ? "없음" : ""))) ?></td>
								<td class="tac"><?= ($row["type"] == "self" ? "셀프관리" : ($row["type"] == "visit" ? "방문관리" : ($row['type'] == 'etc' ? "기타" : ""))) ?></td>
								<td class="tac"><?= $p_row['code_name'] ?></td>
								<td class="tac"><a
                                            href="join_write.php?idx=<?= $row['idx'] ?>"><?= $row["user_name"] ?></a></td>
								
								<td class="tac"><?= $row["tel"] ?></td>
								<td class="tac"><?= ($row["cuckoo"] == 'Y' ? "사용중" : ($row["cuckoo"] == 'N' ? "사용안함" : "")) ?></td>
								<td class="tac"><?= $row['etc'] ?></td>
								<td class="tac"><?= $row["r_date"] ?></td>
								<td class="tac">
									<select name="status" id="status"
                                            onchange="status_chg('<?= $row['idx'] ?>', this.value)">
                                    ...
                                    <!--
								<td class="tac">
									<select name="deli_status" id="deli_status" onchange="deli_chg('<?= $row['idx'] ?>', this.value)">
							...
							-->
                                <td scope="row" class="text-center">
                                    <a href="join_write.php?idx=<?= $row['idx'] ?>"
                                        class="btn btn-primary"><i class="bi bi-pencil"></i></a>
                                    <a href="javascript:del_it(<?= $row['idx'] ?>)" class="btn btn-danger"><i
                                            class="bi bi-trash"></i></a>
                                </td>
								<!-- <td class="tac">
									<a href="join_write.php?idx=<?= $row['idx'] ?>"><img
                                                src="/AdmMaster/_images/common/ico_setting2.png"></a>
					...
					<? echo ipageListing($pg, $nPage, $g_list_rows, $_SERVER[PHP_SELF] . "?s_status=$s_status&search_category=$search_category&search_name=$search_name" . $status_pg . $deli_pg . "&type=$type&pg=") ?>
