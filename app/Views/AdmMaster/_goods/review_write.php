...
<!-- 
				...
				<table cellpadding="0" cellspacing="0" summary="" class="listTable mem_detail" style="table-layout:fixed;">
					...
					<?
					$gift_num = 1;
					foreach($gift_arr as $key => $vals){
						$gift_sql = "
							select * from tbl_freebies a
								left outer join tbl_freebies_code b
									on a.category = b.code_no
								where a.idx = '".$vals."'
								order by a.onum desc, a.idx desc
						";
						$gift_result = mysqli_query($connect, $gift_sql);
						$gift_row = mysqli_fetch_array($gift_result);
					?>
						<tr height="45">
							<th>선택한 사은품<?=$gift_num?></th>
							<td colspan='3'>
							<?
								echo " [ ".$gift_row['code_name']." ] ".$gift_row['subject'];
							?>
							</td>
						</tr>
					<?
