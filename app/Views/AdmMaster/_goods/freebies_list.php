...
<?
	...
	$total_sql = " select * from tbl_freebies where 1=1 and status != 'del' $stateSql $strSql $typeSql";
...
?>
	<div class="page-heading mb-4">
		...
		<div class="d-flex justify-content-between align-items-center">
			...
			<h4 class="text-center">사은품정보 리스트</h4>
		</div>
	</div>
				...
				<div class="inner">
					...
					<div class="menus">
						...
						<ul class="last">
							...
							<li><a href="freebies_write.php" class="btn btn-primary"><span class="glyphicon glyphicon-pencil"></span> <span class="txt">상품 등록</span></a></li>
						</ul>
					...
					<table cellpadding="0" cellspacing="0" summary="" class="listTable01" style="table-layout:fixed;">
						...
						<tbody>
							<tr>
								...
								<td class="inbox">
								<?
									$code_sql = "select * from tbl_freebies_code where status = 'Y' order by onum desc, code_idx desc";
								...
								<td class="inbox">
									...
									<div class="r_box d-flex" style="gap: 10px;">
										<select id="" name="search_category" class="form-select" style="width:112px; height: 39px">
											...
											<option value="subject" <?if($search_category=="subject")echo"selected";?> >제품명</option>
										</select>
						...
						<tbody>
							...
							?>
								...
								<tr>
									...
									<td><a href="freebies_write.php?idx=<?=$row['idx']?>"><span class="edit_btn">EDIT</span></a></td>
									
									<td class="images">
										<div>
											<?if($row["ufile"]){?>
											<img src="/data/product/<?=$row["ufile"]?>" alt="제품 이미지">
											<?}?>
										</div>
									</td>
									<td class="product_name">
										<p><strong>상품명</strong> : <span><?=$row['subject']?></span></p>
										
										<p><strong>사은품카테고리</strong> : <span><?=category_name($row['category'])?></span></p>

									</td>
									
									<td>
									
										<input type="text" name="onum[]" value="<?=$row["onum"]?>" class="form-control input_txt" style="width:80px; text-align: center" />
									...
									<td>
										<input type="checkbox" 
											...
											value="Y"
											data-id="<?= $row['idx'] ?>"
											<?php if($row['is_best'] == 'Y'){ echo "checked"; } ?>>
									</td>
...
<script>
$(document).on("change", ".is_best", function () {
    let id = $(this).data("id");
