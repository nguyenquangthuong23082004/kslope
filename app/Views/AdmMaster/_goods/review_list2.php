...
?>
	<div class="page-heading mb-4">
		...
		<div class="d-flex justify-content-between align-items-center">
			...
			<h4 class="text-center">상품후기(링크)</h4>
		</div>
	</div>
		...
		<span id="print_this"><!-- 인쇄영역 시작 //-->
			...
			<header id="headerContainer">
				...
				<div class="inner">
					<!-- <h2>상품후기(링크) <br>cuckoo24mall.com/contact/review_write_link.php</h2> -->
					<div class="menus">
						<ul class="first">
								...
								<td class="inbox" colspan="3">
									...
									<div class="contact_btn_box">
										<div>
											...
											<input type="text" name="s_date" id="s_date" value="<?=$s_date?>" class="datepicker form-control" style="height: 39px" ><span>~</span><input type="text" name="e_date" id="e_date" value="<?=$e_date?>" class="datepicker form-control" style="height: 39px" >
											<div id="time_layer" style="float: left; display: <?= (trim($s_time) == "" && trim($e_time) == "" ? "none" : "") ?>;">
												<select id="s_time" name="s_time" lass="form-select" style="height: 39px">
													<option value="">선택</option>
													<?php for($t=1; $t<=23; $t++) { ?>
														<option value="<?=$t?>" <?=((int)($s_time) == $t ? "selected" : "")?> ><?=((int)($t) < 10 ? "0" . (int)($t) : (int)($t))?></option>
													<?php } ?>
												</select> ~
												<select id="e_time" name="e_time" lass="form-select" style="height: 39px">
													<option value="">선택</option>
													<?php for($t=1; $t<=23; $t++) { ?>
						...
						<tbody>
							...
							?>
								...
								<tr>
									...
									<td>
										<a href="review_write2.php?idx=<?=$row['idx']?>"><?=$row['user_name']?></a>
									</td>
									<td>
										<p><?=$row['tel1']?></p>
									</td>
									...
									<td>
										<select name='status[]'>
											...
											<option value="Y" <?if($row['status'] == "R")echo"selected";?> >승인거부</option>
										</select>
									</td>
									<td class="tac">
									...
									<td scope="row" class="text-center">
										<a href="review_write2.php?idx=<?=$row['idx']?>"
											class="btn btn-primary"><i class="bi bi-pencil"></i></a>
										<a href="javascript:del_it(<?= $row['idx'] ?>)" class="btn btn-danger"><i
												class="bi bi-trash"></i></a>
									</td>
									<!-- <td>
										<a href="review_write2.php?idx=<?=$row['idx']?>"><img src="/AdmMaster/_images/common/ico_setting2.png"></a>
...
function get_excel_chk(){
	...
	frm.method = "post";
	frm.action = "./excel_down_chk2.php";
	frm.submit();
