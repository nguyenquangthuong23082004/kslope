<?
	...
	for ($i=0;$i < count($m_idx) ; $i++) {
		...
		*/
		...
		$total_sql = " select * from tbl_member_adminrator where m_idx = '".$m_idx[$i]."' ";
		...
		$sql_su = "
				insert into tbl_member_adminrator_out SET 
					 user_id	= '".$user_id."'
					,type		= '".$type."'
					,comment	= '".$comment."'
					,insertDate		=now()
		...
		$sql_su = " update tbl_member_adminrator set status = 'N', d_date = now() where user_id = '".$user_id."'";
