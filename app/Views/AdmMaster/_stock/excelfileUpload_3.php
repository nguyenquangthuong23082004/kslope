	...
	if(is_uploaded_file($UpFile["tmp_name"])) {
	  ...
	  //시트의 지정된 범위 데이터를 모두 읽어 배열로 저장
		...
		for ($i = 3 ; $i <= $maxRow ; $i++) {
			...
			$price_normal = $objWorksheet->getCell("S" . $i)->getValue();
			$price_margin = $objWorksheet->getCell("T" . $i)->getValue();
			$price_one = $objWorksheet->getCell("U" . $i)->getValue();	
			...
			for( $a="F"; $a != "R"; $a++ ) {
				...
				if($code != ""){
					...
					//남성의류 하의 ################################################
					$sql = " select code_no from tbl_size where code_name='".$size."' and type='13' limit 0,1 ;";
					$result = mysqli_query($connect, $sql) or die (mysql_error());
