...
include $_SERVER[DOCUMENT_ROOT]."/include/lib.inc.php";
	...
	if ($search_keyword != "")
	{
		$shere = $shere." and (c.goods_code like '%".$search_keyword."%' OR c.goods_name_front like '%".$search_keyword."%' ) ";
	}

	$total_sql = " select * from tbl_goods_agency_option as a ";
	$total_sql = $total_sql. "		inner join tbl_market as b ";
	$total_sql = $total_sql. "			on a.m_idx= b.m_idx";
	$total_sql = $total_sql. "		inner join tbl_goods as c ";
	$total_sql = $total_sql. "			on a.goods_code = c.goods_code where c.item_state != 'dele' $shere";
	$total_sql = $total_sql. "  group by a.goods_code ";
	...
	$sql    = $total_sql . " order by c.g_idx desc limit $nFrom, $g_list_rows ";
	...
	?>
		...
		?>
			<tr>
				...
				<td class="images"><img src="/data/product/<?=$row["ufile1"]?>" alt="<?=$row["goods_code"]?>" onclick="fncItemClick('<?=$row['g_idx']?>', '<?=$row['product_option']?>','<?=$row["goods_code"]?>');" style="cursor:pointer;" ></td>
			</tr>
