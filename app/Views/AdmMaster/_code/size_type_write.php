...
<?
	...
	if ($code_idx) {
		$total_sql			= " select * from tbl_size_type where code_idx='".$code_idx."'";
		...
		$total_sql			= "select * from tbl_size_type where code_no='".$parent_code_no."'";
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$depth				= $row["depth"]+1;
		...
		$total_sql			= " select ifnull(max(code_no),'".$s_parent_code_no."00')+1 as code_no from tbl_size_type ";
		//echo $total_sql;
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$code_no			= $row["code_no"];
...
<div id="container" class="gnb_code"> <span id="print_this"><!-- 인쇄영역 시작 //-->
	...
	<form name="frm" action="size_type_write_ok.php"  method="post" enctype="multipart/form-data"  target="hiddenFrame" >
...
<script>
		function del_it() {
			if(confirm(" 삭제후 복구하실수 없습니다. \n\n 삭제하시겠습니까?")) {
				hiddenFrame.location.href = "size_type_del.php?code_idx[]=<?=$code_idx?>&mode=view&s_ca_idx=<?=$ca_idx?>";
			}
