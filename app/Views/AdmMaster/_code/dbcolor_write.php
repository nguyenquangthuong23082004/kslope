...
<?
	...
	if ($code_idx) {
		$total_sql			= " select * from tbl_dbcolor where code_idx='".$code_idx."'";
		...
		$total_sql			= "select * from tbl_dbcolor where code_no='".$parent_code_no."'";
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$depth				= $row["depth"]+1;
		...
		$total_sql			= " select ifnull(max(code_no),'".$s_parent_code_no."00')+1 as code_no from tbl_dbcolor ";
		//echo $total_sql;
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$code_no			= $row["code_no"];
...
function send_it()
{
	...
	{
		frm.code_name.focus();
		alert("색상 명을 입력하셔야 합니다.");
		return;
	}

	if (frm.color_code.value == "")
	{
		frm.color_code.focus();
		alert("색상 코드를 입력하셔야 합니다.");
		return;
	}
...
<div id="container" class="gnb_code"> <span id="print_this"><!-- 인쇄영역 시작 //-->
	...
	<form name="frm" action="dbcolor_write_ok.php"  method="post" enctype="multipart/form-data"  target="hiddenFrame" >
				...
				<table cellpadding="0" cellspacing="0" summary="" class="listTable mem_detail">
					...
					<tbody>
						...
						<tr height=45>
							<th>색상 코드</th>
							<td>
								<input type="text" id="color_code" name="color_code" value="<?=$color_code?>" class="input_txt" style="width:30%" />
							</td>
						</tr>
						<tr height=45>
							<th>현황</th>
...
<script>
		function del_it() {
			if(confirm(" 삭제후 복구하실수 없습니다. \n\n 삭제하시겠습니까?")) {
				hiddenFrame.location.href = "dbcolor_del.php?code_idx[]=<?=$code_idx?>&mode=view&s_ca_idx=<?=$ca_idx?>";
			}
