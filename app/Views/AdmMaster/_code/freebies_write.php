...
<?
	...
	if ($code_idx) {
		$total_sql			= " select * from tbl_freebies_code where code_idx='".$code_idx."'";
		...
		$total_sql			= "select * from tbl_freebies_code where code_no='".$parent_code_no."'";
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$depth				= $row["depth"]+1;
		...
		$total_sql			= " select ifnull(max(code_no),'".$s_parent_code_no."00')+1 as code_no from tbl_freebies_code ";
		//echo $total_sql;
		$result				= mysqli_query($connect, $total_sql) or die (mysql_error());
		$row				= mysqli_fetch_array($result);
		$code_no			= $row["code_no"];
...
<div class="page-heading mb-4">
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <h4 class="text-center">사은품코드 <?=$titleStr?> </h4>
    </div>
</div>
...
<div id="container" class="gnb_code"> <span id="print_this"><!-- 인쇄영역 시작 //-->
	...
	<form name="frm" action="freebies_write_ok.php"  method="post" enctype="multipart/form-data"  target="hiddenFrame" >
