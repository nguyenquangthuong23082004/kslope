...
$total_sql = " 
		select sum(cnt) as cnt from tbl_location_addr
	";
	$result = mysqli_query($connect, $total_sql) or die (mysql_error());
	$row = mysqli_fetch_array($result);
	$cnt_tot		= $row[cnt];

	$sql = " select * from tbl_location_addr where 1=1 ";
...
<?
					...
					$sql = $sql . " order by cnt desc, location desc limit $nFrom, $g_list_rows ";
															
					$result = mysqli_query($connect, $sql) or die (mysql_error());
					$num = $nTotalCount - $nFrom;

					while($row=mysqli_fetch_array($result)){
						$total = ($row["cnt"]/$cnt_tot * 100);
