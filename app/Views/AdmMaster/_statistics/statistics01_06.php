...
$sWhere = "";
if($pay_device == "P"){
	$sWhere = " and a.pay_device = 'P' ";
}else if($pay_device == "M"){
	$sWhere = " and a.pay_device = 'M' ";
}



if($sDate != "" && $eDate != ""){
	$sWhere = $sWhere. " and (DATE(a.regdate) >= '".$sDate."' AND DATE(a.regdate) <= '".$eDate."')";
}
	...
	$sql = "
		...
		from (
		select 
		case DAYOFWEEK(regdate) when 1 then 1 end as A
		,case DAYOFWEEK(regdate) when 2 then 1 end as B
		,case DAYOFWEEK(regdate) when 3 then 1 end as C
		,case DAYOFWEEK(regdate) when 4 then 1 end as D
		,case DAYOFWEEK(regdate) when 5 then 1 end as E
		,case DAYOFWEEK(regdate) when 6 then 1 end as F
		,case DAYOFWEEK(regdate) when 7 then 1 end as G
		from tbl_order as a where a.status='M' ".$sWhere." 
		) tb
	";
			...
			<div id="contents">
				...
				<div class="listWrap statis01">
					...
					<div class="listBottom" style="margin-top:-10px;">
						<table cellpadding="0" cellspacing="0" summary="" class="listTable statisTable statisTable06">
						<caption></caption>
						<colgroup>
						<col width="14.28%" />
						<col width="14.28%" />
						...
						<tbody>
							<tr>
								<td>
									<div>
										<div class="graph02" data-percent="<?=$a_av?>%"><span><?=$a_av?>%</span></div>
									</div>
									<p><?=number_format($a)?>건</p>			
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$b_av?>%"><span><?=$b_av?>%</span></div>
									</div>
									<p><?=number_format($b)?>건</p>
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$c_av?>%"><span><?=$c_av?>%</span></div>
									</div>
									<p><?=number_format($c)?>건</p>
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$d_av?>%"><span><?=$d_av?>%</span></div>
									</div>
									<p><?=number_format($d)?>건</p>
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$e_av?>%"><span><?=$e_av?>%</span></div>
									</div>
									<p><?=number_format($e)?>건</p>
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$f_av?>%"><span><?=$f_av?>%</span></div>
									</div>
									<p><?=number_format($f)?>건</p>
								</td>
								<td>
									<div>
										
										<div class="graph02" data-percent="<?=$g_av?>%"><span><?=$g_av?>%</span></div>
									</div>
									<p><?=number_format($g)?>건</p>
								</td>
							</tr>
