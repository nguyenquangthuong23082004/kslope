...
$years  = $_GET['years'] ?? $current_year;
$months = $_GET['months'] ?? $current_month;
...
function getWeeksOfMonth($year, $month) {
    ...
    for ($i = $start; $i <= $end; $i += 86400) {
        $day_of_week = date('N', $i); // 1 = Mon, 7 = Sun
...
$week = isset($_GET['week']) && $_GET['week'] !== '' ? (int)$_GET['week'] : $current_week_idx + 1;

$target_week = $week_arr[$week - 1] ?? $week_arr[0];
$s_date = $target_week['start'];
$e_date = $target_week['end'];
    ...
    <span id="print_this">
        <div id="mainContentMenu" class="contentMenu">
            <ul>
                ...
                <li class="contentMenuSub selected"><a href="member_statistics4_week.php">검색어통계(주간)</a></li>
        ...
        <div id="contents">
            <div class="content">
                ...
                <div class="d-flex justify-content-between">
                        ...
                        <form name="modifyForm1" method="get" action="" autocomplete="off">
                            <div class="firstLine selectYear d-flex gap-2" style="padding-left:0">
                                ...
                                <select name="week" onchange="fn_search()" class="form-select">
                                    ...
                                    <?php foreach($week_arr as $idx => $w): ?>
                                        <option value="<?= $idx+1 ?>" <?= ($week==($idx+1))?"selected":"" ?>>
                                            <?= $idx+1 ?>주 (<?= $w['start'] ?> ~ <?= $w['end'] ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                    ...
                    <div class="listSelectR">
                        <div class="contentMenu">
                            <ul>
                                <li class="contentMenuSub " data-mode="year"><a href="member_statistics4_year.php">년간통계</a></li>
                                <li class="contentMenuSub " data-mode="month"><a href="member_statistics4_month.php">월간통계</a></li>
                                <li class="contentMenuSub selected" data-mode="week"><a href="member_statistics4_week.php">주간통계</a></li>
                ...
                <div id="listArea">
                    <h4><?= $s_date ?> ~ <?= $e_date ?> (총 <?= number_format($total_cnt) ?>회)</h4>
