...
$days_in_month = date('t', strtotime("$years-$months-01"));

$day_arr  = array_fill(1, $days_in_month, 0);
$day_arr2 = array_fill(1, $days_in_month, 0);

$s_date = date('Y-m-01 00:00:00', strtotime("$years-$months-01"));
$e_date = date('Y-m-t 23:59:59', strtotime($s_date));

$sql1 = "SELECT DAY(r_date) AS day_num, COUNT(*) AS tcnt
         FROM tbl_member 
         WHERE r_date >= '$s_date' AND r_date <= '$e_date' AND status='1' AND user_level>2
         GROUP BY day_num";
$res1 = mysqli_query($connect, $sql1);
while ($row = mysqli_fetch_assoc($res1)) {
    $day_arr[(int)$row['day_num']] = (int)$row['tcnt'];
}
$_total_cnt = array_sum($day_arr);

$sql2 = "SELECT DAY(r_date) AS day_num, COUNT(*) AS tcnt
         FROM tbl_member 
         WHERE r_date >= '$s_date' AND r_date <= '$e_date' AND status='0' AND user_level>2
         GROUP BY day_num";
$res2 = mysqli_query($connect, $sql2);
while ($row = mysqli_fetch_assoc($res2)) {
    $day_arr2[(int)$row['day_num']] = (int)$row['tcnt'];
}
$_total_cnt2 = array_sum($day_arr2);
?>
            ...
            <div class="content">
                ...
                <div class="d-flex justify-content-between">
                    <div class="listSelect size09">
                        <form name="modifyForm1" method="get" action="member_statistics_day.php" autocomplete="off">
                        ...
                        function drawChart() {
                            var data = google.visualization.arrayToDataTable([
                                ...
                                for($i=1;$i<=$days_in_month;$i++){
                                    $date_str = date('Y-m-d', strtotime("$years-$months-$i"));
                                    echo "['$date_str', ".$day_arr[$i].", ".$day_arr2[$i]."],";
                                }
                                ?>
                            ]);
                    ...
                    <table class="listIn fixed-header">
                        ...
                        <tbody>
                            <?php for($i=1;$i<=$days_in_month;$i++): ?>
                                <?php $date_str = date('Y-m-d', strtotime("$years-$months-$i")); ?>
                                <tr>
                                    <td><?= $date_str ?></td>
                                    <td><?= $day_arr[$i] ?> <span><?= ($_total_cnt>0)?round($day_arr[$i]/$_total_cnt*100,2):0 ?>%</span></td>
                                    <td><?= $day_arr2[$i] ?> <span><?= ($_total_cnt2>0)?round($day_arr2[$i]/$_total_cnt2*100,2):0 ?>%</span></td>
                                </tr>
                            <?php endfor; ?>
                        </tbody>
                    </table>
