...
if (empty($days)) {
    $s_date = date('Y-m-01', strtotime("$years-$months-01"));
    $e_date = date('Y-m-t', strtotime($s_date));
} else {
    $s_date = date('Y-m-d', strtotime("$years-$months-$days"));
    $e_date = $s_date;
}
...
$sql = "
    SELECT DATE(regdate) AS time_unit, SUM($sum_col) AS tcnt
    FROM tbl_login_device
    WHERE regdate >= '$s_date 00:00:00' AND regdate <= '$e_date 23:59:59'
    GROUP BY DATE(regdate)
";

$result = mysqli_query($connect, $sql);

$start_ts = strtotime($s_date);
$end_ts = strtotime($e_date);
$day_arr = [];
while ($start_ts <= $end_ts) {
    $day_str = date('Y-m-d', $start_ts);
    $day_arr[$day_str] = 0;
    $start_ts = strtotime('+1 day', $start_ts);
}

while ($row = mysqli_fetch_assoc($result)) {
    $day_arr[$row['time_unit']] = (int)$row['tcnt'];
}
            ...
            <div class="content">
                ...
                <div class="d-flex justify-content-between">
                    <div class="listSelect size09">
                        <form name="modifyForm1" method="get" action="member_statistics3_day.php" autocomplete="off">
                            ...
                            <div class="firstLine selectYear d-flex gap-2" style="padding-left:0">
                                ...
                                <select name="days" onchange="fn_search()" class="form-select">
                                    ...
                                    <?php
                                    $num_days = date('t', strtotime("$years-$months-01"));
                                    for ($d = 1; $d <= $num_days; $d++) {
                                        $day_str = date('Y-m-d', strtotime("$years-$months-$d"));
                                        $selected = ($days == $d) ? "selected" : "";
                                        echo "<option value='$d' $selected>$day_str</option>";
                                    }
                                    ?>
                                </select>
                    ...
                    <script type="text/javascript">
                        ...
                        function drawChart() {
                            var data = google.visualization.arrayToDataTable([
                                ['날짜', '방문자수'],
                                <?php foreach ($day_arr as $day => $cnt): ?>
                                    ['<?= $day ?>', <?= $cnt ?>],
                                <?php endforeach; ?>
                            ]);
