...
<?php
$years_selected = $_GET['years'] ?? date('Y');
$first_year = 2024;
$last_year = date('Y');
...
for ($y = $first_year; $y <= $last_year; $y++) {
    $start_year = date('Y-01-01 00:00:00', strtotime("$y-01-01"));
    $end_year   = date('Y-12-31 23:59:59', strtotime("$y-01-01"));
    $year_labels[] = $y;

    $sql1 = "SELECT COUNT(*) AS cnt
             FROM tbl_member
             WHERE r_date >= '$start_year' AND r_date <= '$end_year' AND status='1' AND user_level>2";
    ...
    $sql2 = "SELECT COUNT(*) AS cnt
             FROM tbl_member
             WHERE r_date >= '$start_year' AND r_date <= '$end_year' AND status='0' AND user_level>2";
            ...
            <div class="content">
                ...
                <div class="d-flex justify-content-between">
                    <div class="listSelect size09">
                        <!-- <form name="modifyForm1" method="get" action="member_statistics_year.php" autocomplete="off">
                            <div class="firstLine selectYear d-flex gap-2" style="padding-left:0">
                                <select name="years" onchange="fn_search()" class="form-select">
                                    <?php for ($y = $first_year; $y <= $last_year; $y++): ?>
                                        <option value="<?= $y ?>" <?= ($y == $years_selected) ? "selected" : "" ?>><?= $y ?>년</option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                        </form> -->
                ...
                <div id="listArea">
                    ...
                    <script type="text/javascript">
                        ...
                        function drawChart() {
                            var data = google.visualization.arrayToDataTable([
                                ['년', '활성', '비활성'],
                                <?php
                                foreach($year_labels as $i => $year) {
                                    echo "['$year', ".$active_counts[$i].", ".$inactive_counts[$i]."],";
                                }
                                ?>
                            ]);
                    ...
                    <table class="listIn fixed-header">
                        ...
                        <tbody>
                            <?php foreach($year_labels as $i => $year): ?>
                                <tr>
                                    <td><?= $year ?></td>
