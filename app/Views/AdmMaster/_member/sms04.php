...
<?
	...
	$types	= updateSQ($_GET["types"]);

	$strSql = "";

	if ($title) {
		$strSql .= " and content like '%".$title."%' ";
	}
						...
						<table cellpadding="0" cellspacing="0" summary="" class="listTable01 email_form01" style="table-layout:fixed;">
							...
							<tbody>
								<tr>
									...
									<td class="inbox">
										<select name="types" id="types">
											...
											<option value="S" <?if($types=="S")echo"selected";?> >단체</option>
											<option value="P" <?if($types=="P")echo"selected";?> >개별</option>
										</select>
									</td>
								...
								<tr>
									...
									<td class="inbox" colspan="3"><input type="text" name="title" value="<?=$title?>" style="width:80%;"></td>
								</tr>
							</tbody>
						</table>
				...
				<div class="listWrap">
					...
					<div class="listBottom">
						...
						?>
							<tr>
								<td><?=$row_num?></td>
								<td><?if($row['types']=="P"){echo "개별";}else if($row['types']=="S"){ echo "단체"; }?></td>
								<td><a href="sms_send.php?code=<?=$row['code']?>"><?=$row['content']?></td>
								<td style="word-break:break-all;"><?=$row['to_hp']?></td>
								<td><?=$row['regdate']?></td>
							</tr>
						<?  } ?>
					...
					<?echo ipageListing($pg, $nPage, $g_list_rows, $_SERVER[PHP_SELF]."?s_date=$s_date&e_date=$e_date&title=$title&types=$types&pg=")?>
