...
$(document).ready(function () {
	$('.inquiry_close').on('click', function () {
		$('.inquiry_box').css({ 'display': 'none' });
		$('.agree_layer').css({ 'display': 'none' });

		$('html').css({ 'overflow': 'auto', 'height': '100%' });
		//$('body').css({'overflow-y': 'auto'});
		$('#element').off('scroll touchmove mousewheel');

		if ($(window).width() < 1200) { $('body').css({ 'overflow-y': 'auto' }); }
	});


	$(".solution_popup").on('click', function () {
		$('.inquiry_box').css({
			'display': 'block',
			'position': 'absolute;'
		});
	...
	$('.inquiry_on').on('click', function () {
		...
		if ($(window).width() < 1200) {
			...
			$('#element').off('scroll touchmove mousewheel');
			$('.inquiry_wrap').css({ 'height': $(window).height() });
		} else {
			//$('html').css({'overflow-y': 'hidden', 'height': '100%'}); 
			//$('body').css({'overflow-y': 'hidden'});
			$('#element').on('scroll touchmove mousewheel', function (event) { event.preventDefault(); event.stopPropagation(); return false; });
	...
	$('.inquiry_box').click(function (e) {

		if ($(e.target).hasClass('inquiry_wrap')) {
			//alert(33);
			$('.inquiry_box').css({ 'display': 'none' });
	...
	/*if($('.inquiry_box').attr('display','block')){
...
$(document).ready(function () {
	...
	$('.quibttn').click(function () {
		$('.quick_link').toggleClass('active');
	})
...
$(document).ready(function () {
	...
	$('#potfolio_gnb ul li').hover(function () {
		$('#potfolio_gnb ul li').removeClass('active');
		$(this).addClass('active');
	});

	$('#news_gnb ul li').hover(function () {
		$('#news_gnb ul li').removeClass('active');
		$(this).addClass('active');
	});
...
document.addEventListener("DOMContentLoaded", function () {
	const scrollBtn = document.querySelector(".floating-btn--scroll");

	window.addEventListener("scroll", function () {
		const y = window.scrollY;

		scrollBtn.style.display = y > 300 ? "flex" : "none";
	});
