...
function fn_addzzim(g_idx, obj){

	$.ajax({
		url: "/item/item_zzim_ajax.php",
		type: "GET",
		data: "g_idx="+g_idx,
		error : function(request, status, error) {
		 //통신 에러 발생시 처리
...
function vs_btn_check(g_idx, obj){
  
	$.ajax({
		url: "/item/item_vs_ajax.php",
		type: "GET",
		data: "g_idx="+g_idx,
		error : function(request, status, error) {
		 //통신 에러 발생시 처리
...
function execDaumPostcode_san(frm_name, zip, addr1, addr2) {
	...
	new daum.Postcode({
		oncomplete: function(data) {
			...
			*/
			...
			var chk_code = data.zonecode.substr(0,2);
			sangan(chk_code);
