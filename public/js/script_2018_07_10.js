function responsive(){
   ...
   if ($wh <= 768)
   {
      ...
      $("body").addClass("mobile");
		...
		$('h1.logo img').attr('src','/img/main/m_logo.png');
...
/*function MainSlider(){
	...
	if ($wh <= 736) {
		var slider02= $('.m_slider_box').bxSlider({
			//mode: 'fade',
			captions: true,
			auto: true,        // 자동 실행 여부
		...
		$('.btn_stop_slide').on('click',function(){
			slider02.stopAuto();
		});
		$('.btn_play_slide').on('click',function(){
			slider02.startAuto();
		});

	}else {

	var slider = $('.slider_box').bxSlider({
			//mode: 'fade',
			captions: true,
			auto: true,        // 자동 실행 여부
			autoHover: true,   // 마우스 호버시 정지 여부
...
$(function(){
	...
	var $slide = $(".brand_list")
	$slide.slick(setting);
	$(".slide_l").click(function(){$("#f_brands .slick-prev.slick-arrow").click();});
	$(".slide_r").click(function(){$("#f_brands .slick-next.slick-arrow").click();});
});
...
/*function Hotnew(){
	 var $wh = $(window).width();
	  ...
	  if ($wh <= 736) {
		var $Slider02 =	$('.hot_right').slick({
				  slidesToShow: 1,
				  slidesToScroll: 1,
				  arrows: false,
					   infinite: false,
				  //fade: true,
				  asNavFor: '.Hot_slider_nav'
				});
				$('.Hot_slider_nav').slick({
				  slidesToShow: 1,
				  slidesToScroll: 1,
				  asNavFor: '.hot_right',
				  //dots: true,
...
function CategoryList(e){
		...
		$('.btn_category').removeClass('on');
			...
			$(".cg_list > li .depth02").mouseenter(function(){
...
$(function(){
	...
	$(document).on("click",".btn_detil_fi,.snb_detail_open .layer_bg,.close_btn",function(){
		if($("body").hasClass("mobile")){
...
$(function(){
	...
	$(".family_btn").click(function(){
		if($(".family_blist").css("display") == "none"){
			$(".family_blist").show();
		}else{$(".family_blist").hide();}
...
//퀵메뉴 스크롤
$( document ).ready( function() {
	var jbOffset = $( "#wrap" ).height() - $("#footer").height();
	$( window ).scroll( function() {
		if ( $( document ).scrollTop() < 200 ) {
			$(".time_sbox").css({"top": "277px"});
			$(".quick_link").css({"top": "278px"});
		} else {
			$(".time_sbox").css({"top": "33px"});
			$(".quick_link").css({"top": "33px"});
		}
	});
});
/*
$( document ).ready( function() {
	var jbOffset = $( "#wrap" ).height() - $("#footer").height() - 300;
	$( window ).scroll( function() {
		if ( $( document ).scrollTop() < jbOffset ) {
			$(".time_sbox").animate({"bottom": "125px"}, 100);
			$(".quick_link").animate({"top": "215px"}, 100);
		} else if($( document ).scrollTop() > jbOffset) {
			$(".time_sbox").animate({"bottom": "316px"}, 100);
			$(".quick_link").animate({"top": "163px"}, 100);
		}
	});
});*/
...
/*function fnMove(seq){
	var offset = $("#retal_order" + seq).offset();
	if($(window).scrollTop() < 745){
		$('html, body').animate({scrollTop : offset.top-383 }, 500);
	}else{
		$('html, body').animate({scrollTop : offset.top-203 }, 500);
	}
